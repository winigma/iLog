...
infosoftglobal.FusionCharts.prototype = {
	...
	encodeDataXML: function(strDataXML){
			...
			var arrSQAtt=strDataXML.match(/=\s*\".*?\"/g);
			if (arrDQAtt){
				for(var i=0;i<arrDQAtt.length;i++){
					var repStr=arrDQAtt[i].replace(/^=\s*\"|\"$/g,"");
					repStr=repStr.replace(/\'/g,"%26apos;");
					strDataXML=strDataXML.replace(arrDQAtt[i],"='"+repStr+"'");								
				}
			}
			if (arrSQAtt){
				for(var i=0;i<arrSQAtt.length;i++){
					var repStr=arrSQAtt[i].replace(/^=\s*\'|\'$/g,"");
					repStr=repStr.replace(/\"/g,"%26quot;");
					strDataXML=strDataXML.replace(arrSQAtt[i],"='"+repStr+"'");								
				}
			}
