...
@Controller("mBeanManterFollowUpExterno")
@AccessScoped
public class MBeanManterFollowUpExterno extends AbstractManterPaginacao {
	...
	private static Logger logger = LoggerFactory
			.getLogger(MBeanManterFollowUpExterno.class);
	...
	public String editar() {
		...
		try {
			...
			List<Invoice> listaInvoices = facade.listarInvoicesByCarga(this.carga, true);
	...
	private void calcularTotaisDias() {
		...
		if (followUp.getTrechosFollowUp() != null
			...
			totalEstimados.add(DataUtils.diferencaEmDias(inicio, fim));
			calcularTotaisDiasEstimados();
			inicio = null;
			fim = null;
	...
	public void carregarFollowUp() throws BusinessException {
		if (carga != null && carga.getRota() != null) {
			...
			if (true) {
				...
				ParametroCanal canal = cadastro.getParametroCanais(Canal.VE);
	...
	public void selecionarItemFollowUp(int index) {
		try {
			...
			if (followTrecho.isCanal() && followTrecho.getParametroCanal() != null ) {
	...
	public void getOcorrenciasTrecho(Cidade cidade) {
        ...
        Long i = cidade.getId();
        try{
		cidadeOcorrencia = cadastro.getCidadeById(i);}catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void verificarFollowUp() {
		...
		if (followTrecho.getDtRealizado() != null
			...
			// carrega a data prevista atual
			try {
				// FollowUpImportTrecho trecho = followUp.getTrechosFollowUp()
				// .get(followUp.getTrechosFollowUp().size() - 1);
			...
			/**
			 * @TODO UIModalPanel modal = (UIModalPanel) RichFunction
			 *       .findComponent("modalItinerario"); if (modal != null) {
			 *       modal.setShowWhenRendered(false);
			 *       AjaxContext.getCurrentInstance()
			 *       .addComponentToAjaxRender(modal); }
			 **/
	 ...
	 * @param ae
	 */
	public void modificarAtual(ActionEvent ae) {
	...
	public void modificarProximos() {
		...
		for (int j = i; j < followUp.getTrechosFollowUp().size(); j++) {
			...
			FollowUpImportTrecho trechoAux = followUp.getTrechosFollowUp().get(
					j);
			FollowUpEstimado estimado = new FollowUpEstimado();
			estimado.setAutor(user);
			estimado.setDtAlteracao(new Date());
			estimado.setFollowUpTrecho(trechoAux);
			estimado.setDtEstimada(trechoAux.getDtPlanejado());
			...
			if (trechoAux.getId() != null
				...
				// ALTERAR O HISTORICO
				FollowUpEstimado followEstimado = mapaEstimado.get(trechoAux
						.getId());
	...
	public String salvar() {
		...
		List<String> errorMessages = ValidatorHelper
				.valida(carga, TemplateMessageHelper.getResourceBundle(
	...
	public String excluir() {
		try {
			...
			for (List<Ocorrencia> listOcorrenciaImport : ocorrencias
					.values()) {
...
private StreamedContent file;
	...
	public void handleFileUpload(FileUploadEvent event) {
		...
		try {
			...
			file = new DefaultStreamedContent(event.getFile()
					.getInputstream());
	...
	public void setCanalEstimadoSelecionado(ParametroCanal canalEstimadoSelecionado) {
